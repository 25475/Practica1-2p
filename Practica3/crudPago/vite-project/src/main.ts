...
(async ()=> {
  ...
  data.forEach((pago: IPago) => {
    divTable += `<tr><td>${pago.id}</td><td>${pago.code}</td><td>${pago.fecha}</td><td>${pago.monto}<button class="btn btn-delete">Eliminar</button></td><td><button class="btn btn-update">Actualizar</button> </td> </tr>`;
  });
  divTable += `</table>`;
  ...
  document.querySelectorAll<HTMLButtonElement>('.btn-delete').forEach(btn=>{
    btn.addEventListener('click', async ()=>{ 
      ...
      console.log(id);
      await fetch(`http://localhost:3000/api/pagos/${id}`, {
        method: 'DELETE'
      });
      location.reload();
  ...
  document.querySelectorAll<HTMLButtonElement>('.btn-update').forEach(btn=>{
    btn.addEventListener('click', async ()=>{ 
      ...
      btnSave?.addEventListener('click', async (e)=>{
        e.preventDefault()
        const code = document.querySelector<HTMLInputElement>('#code')!.value;
        const fecha = document.querySelector<HTMLInputElement>('#fecha')!.value;
        const response = await fetch(`http://localhost:3000/api/pagos/${id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({code, fecha, facturaId:1})
        });
        const data = await response.json();
        console.log(data);
        alert (data);
